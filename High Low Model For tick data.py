...
dat = pd.read_csv("C:\LOG/NOKSEK 15 Min Data.csv")
...
for i in range(uniq_dates.shape[0]-1):
    if(pd.to_datetime(uniq_dates[0].loc[i]).weekday()==4):## on friday
        temp = dat.loc[(dat.index>=pd.to_datetime(uniq_dates[0].loc[i]+close_time))&(dat.index<=pd.to_datetime(uniq_dates[0].loc[i+2]+close_time))]
    else:
        temp = dat.loc[(dat.index >= pd.to_datetime(uniq_dates[0].loc[i] + close_time)) & (
                    dat.index <= pd.to_datetime(uniq_dates[0].loc[i + 1] + close_time))]
...
for i in range((uniq_dates.shape[0]-1)):
    HC_TP=0
    LC_TP=0
    if (pd.to_datetime(uniq_dates[0].loc[i]).weekday() == 4):
        temp = dat.loc[(dat.index >= pd.to_datetime(uniq_dates[0].loc[i] + close_time)) & (
                    dat.index <= pd.to_datetime(uniq_dates[0].loc[i + 2] + close_time))]
    else:
        temp = dat.loc[(dat.index >= pd.to_datetime(uniq_dates[0].loc[i] + close_time)) & (
                dat.index <= pd.to_datetime(uniq_dates[0].loc[i + 1] + close_time))]
    ...
    if (~temp['HC_Ret'].values.any()):
        temp.iloc[m:(m+1),4:5]=float(temp.iloc[m:(m+1),0:4].values.mean()/entry_val-1)
    ...
    if (~temp['LC_Ret'].values.any()):
        temp.iloc[m:(m+1),5:6]=-1*(float(temp.iloc[m:(m+1),0:4].values.mean()/entry_val-1))

    tt=pd.DataFrame(data=[temp.index[0],entry_val,temp['HC_Ret'].loc[temp['HC_Ret']!=0].index[0],
                          temp['HC_Ret'].loc[temp['HC_Ret']!=0].values[0],temp['LC_Ret'].loc[temp['LC_Ret']!=0].
                    index[0],temp['LC_Ret'].loc[temp['LC_Ret']!=0].values[0]]).T
    tt.columns=['Entry Date','Entry_Price','Long_Exit','Long_Ret','Short_Exit','Short_Ret']
    final_dat=final_dat.append(tt, ignore_index=True)

final_dat.to_csv('C:/Users/Kaushtuv/Desktop/final_dat.csv')
