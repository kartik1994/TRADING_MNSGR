...
def transmit_entry(curr_name, qty, contract, ids, round1, signal, tot_time, fav_time, mid_time,tickerId,closetime):
        ...
        for i in range(tot_time):
            ...
            if week_day != 4 and "NZD" in curr_name:
                if "12:59:50" <= currtime[-8:] <= "13:15:05":
                    sleeptime = (datetime.strptime("13:15:10", '%H:%M:%S') - datetime.strptime(currtime[-8:],'%H:%M:%S')).total_seconds()
                    print(curr_name + " sleeping till mkt opens at 13:15")
                    time.sleep(sleeptime)  # sleeptime

            else:
                if "16:59:50" <= currtime[-8:] <= "17:15:05":
                    sleeptime = (datetime.strptime("17:15:10", '%H:%M:%S') - datetime.strptime(currtime[-8:],'%H:%M:%S')).total_seconds()
...
london_df=pd.DataFrame({"CURRENCY":london_list,"OPEN_POSITION":np.nan,"ENTRY_QTY":np.nan,"ENTRY_SIGNAL":np.nan,"ASK":np.nan,"BID":np.nan,
...
ny_df=pd.DataFrame({"CURRENCY":ny_list,"OPEN_POSITION":np.nan,"ENTRY_QTY":np.nan,"ENTRY_SIGNAL":np.nan,"ASK":np.nan,"BID":np.nan,
                        "ENTRY_EXE":np.nan,"ENTRY_TIME":np.nan,
                        "LONG_EXIT_QTY":np.nan,"LONG_TP":np.nan,"LONG_EXIT_EXE":np.nan,"LONG_EXIT_TIME":np.nan,
                        "SHORT_EXIT_QTY":np.nan,"SHORT_TP":np.nan,"SHORT_EXIT_EXE":np.nan,"SHORT_EXIT_TIME":np.nan,
                        "CLOSE":np.nan,"ACT_RETURN":np.nan,"CYCLE_DATE":np.nan})
...
def LONDON(params, ids,tickerId,histId,london_df):
    tp_buy = 0
    tp_buy2 = 0
    tp_sell = 0
    tp_sell2 = 0
    ...
    entry_result = transmit_entry(params[0], params[2], contract, ids, round1, params[1], params[3], params[4],
    ...
    #
    # if currtime < (entry_result[-1] + ' ' + (closetime + timedelta(seconds=60)).strftime("%H:%M:%S")):
    #     cond= "NZD" in params[0]
    #     if cond==True:
    #         sleeptime = ((closetime+timedelta(minutes=15)) - datetime.strptime(currtime[-8:],
    #                                                                              '%H:%M:%S')).total_seconds()
    #
    #         print(params[0], "sleeping til 23:45")
    #     else:
    #         sleeptime = ((closetime + timedelta(seconds=62)) - datetime.strptime(currtime[-8:],
    #                                                                              '%H:%M:%S')).total_seconds()
    #         print(params[0],"sleeping til 23:31")
    #
    #
    ...
    if london_list.index(params[0]) <= 4:
        ...
        if entry_result[0] > 0:
            ...
            tp_sell2 = round(float(((asset_hist_data['close'] + (tp2 * abs((pd.Series.rolling(
                (asset_hist_data['high'].shift(-1) / asset_hist_data['close'] - 1).shift(1),
                window=lookback).mean()).dropna()) * asset_hist_data['close'])).dropna()).iloc[-1]), round_tp)
            ############## END OF TP CALCULATION ##############################################################################


    print("@@@@@@@@@@  TP FOR",params[0],"IS",tp_buy,tp_sell," @@@@@@@@@@@@@")
    time.sleep(5)
    dat = open_pos()
    ...
    if (dat.empty) or (dat[(dat["Local Symbol"] == params[0])].empty) or (
        ...
        while pos != 0:
            ...
            if week_day == 4 and currtime[-8:] >= "16:59:50":
                ...
                sleeptime = (datetime.strptime(wakeuptime, "%Y%m%d %H:%M:%S") - datetime.strptime(currtime,"%Y%m%d %H:%M:%S")).total_seconds()
                ...
                if "NZD" in params[0]:
                    if "12:59:50" <= currtime[-8:] <= "13:15:05":
                        sleeptime = (
                        datetime.strptime("13:15:10", '%H:%M:%S') - datetime.strptime(currtime[-8:], '%H:%M:%S')).total_seconds()
                        print(params[0],"sleeping till mkt opens at 13:15")
                        time.sleep(sleeptime)#sleeptime
                    ...
                    if "16:59:50" <= currtime[-8:] <= "17:15:05":
                        sleeptime = (
                        datetime.strptime("17:15:10", '%H:%M:%S') - datetime.strptime(currtime[-8:], '%H:%M:%S')).total_seconds()
                        print(params[0],"sleeping till mkt opens at 17:15")
                        time.sleep(sleeptime)#sleeptime
    ...
    path="D:\REPORTS/"+entry_result[-1] + "_LONDON.xlsx"
...
def NY(params, ids,tickerId,histId,ny_df):
    tp_buy = 0
    tp_buy2 = 0
    tp_sell = 0
    tp_sell2 = 0
    ...
    # asset_hist_data = asset_hist_data[:-1]
    close_data=asset_hist_data["close"].iloc[-1]
    close_data=round(float(close_data), 4)
    ...
    if (dat.empty) or (dat[(dat["Local Symbol"] == params[0])].empty) or (
        ...
        while pos != 0:
            ...
            if week_day == 4 and currtime[-8:] >= "16:59:50":
                wakeuptime = ((datetime.now() + timedelta(+2)).strftime("%Y%m%d")) + ' 17:15:10'
                sleeptime = (datetime.strptime(wakeuptime, "%Y%m%d %H:%M:%S") - datetime.strptime(currtime,"%Y%m%d %H:%M:%S")).total_seconds()
                print('sleeping till monday 17:15')
                print("wakeup at ",wakeuptime)
                time.sleep(sleeptime)
            else:
                if not "NZD" in params[0]:
                    if "16:59:50" <= currtime[-8:] <= "17:15:10":
                        sleeptime = (
                        datetime.strptime("17:15:10", '%H:%M:%S') - datetime.strptime(currtime[-8:], '%H:%M:%S')).total_seconds()
                        print("sleeping till mkt opens at 17:15")
                        time.sleep(sleeptime)#sleeptime
    ...
    path = "D:\REPORTS/" + entry_result[-1] + "_NY.xlsx"
    writer = pd.ExcelWriter(path)
...
def run_london():
    ...
    if shortsum > 1 and longsum >1:
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.NZD', 'TP1'].iloc[-1] <= 0.25) and \
            ...
            Thread(target=LONDON, args=(AUDNZD_params, ids,1,2,london_df)).start()
        ################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.NOK', 'TP2'].iloc[-1] <= 0.25) and (switchflag1 in ('yes','YES')):
            Thread(target=LONDON, args=(CHFNOK_params, ids1,3,4,london_df)).start()
        ###############################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.SEK', 'TP2'].iloc[-1] <= 0.25) and (switchflag2 in ('yes','YES')):
            Thread(target=LONDON, args=(CHFSEK_params, ids2,5,6,london_df)).start()
        ##########################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'NZD.CHF', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'NZD.CHF', 'TP2'].iloc[-1] <= 0.25) and (switchflag3 in ('yes','YES')):
            Thread(target=LONDON, args=(NZDCHF_params, ids3,7,8,london_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.SEK', 'TP2'].iloc[-1] <= 0.25) and (switchflag4 in ('yes','YES')):
            Thread(target=LONDON, args=(GBPSEK_params, ids4,9,10,london_df)).start()
        ###############################################################    
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.NZD', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.NZD', 'TP2'].iloc[-1] <= 0.25) and (switchflag5 in ('yes','YES')):
            Thread(target=LONDON, args=(EURNZD_params, ids5,11,12,london_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.NOK', 'TP2'].iloc[-1] <= 0.25) and (switchflag6 in ('yes','YES')):
             Thread(target=LONDON, args=(EURNOK_params, ids6,12,13,london_df)).start()
        ##############################################    
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.SEK', 'TP2'].iloc[-1] <= 0.25) and (switchflag7 in ('yes','YES')):
            Thread(target=LONDON, args=(EURSEK_params, ids7,14,15,london_df)).start()
        ##################################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.NOK', 'TP2'].iloc[-1] <= 0.25) and (switchflag8 in ('yes','YES')):
            Thread(target=LONDON, args=(GBPNOK_params, ids8,16,17,london_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'NOK.SEK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'NOK.SEK', 'TP2'].iloc[-1] <= 0.25) and (switchflag9 in ('yes','YES')):
            Thread(target=LONDON, args=(NOKSEK_params, ids9,18,19,london_df)).start()
        ########################
    return
...
def run_ny():
    ...
    if shortsum > 1 and longsum > 1:
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.SGD', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.SGD', 'TP2'].iloc[-1] <= 0.25) and (switchflag in ('yes', 'YES')):
            Thread(target=NY, args=(AUDSGD_params, audsgdids,40,41,ny_df)).start()
        ##################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.PLN', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'CHF.PLN', 'TP2'].iloc[-1] <= 0.25) and (switchflag1 in ('yes', 'YES')):
            Thread(target=NY, args=(CHFPLN_params, chfplnids,20,21,ny_df)).start()
        ##################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.CZK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.CZK', 'TP2'].iloc[-1] <= 0.25) and (switchflag2 in ('yes', 'YES')): 
            Thread(target=NY, args=(GBPCZK_params, gbpczkids,22,23,ny_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.PLN', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.PLN', 'TP2'].iloc[-1] <= 0.25) and (switchflag3 in ('yes', 'YES')):
            Thread(target=NY, args=(GBPPLN_params, gbpplnids,24,25,ny_df)).start()
        ##################################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.ZAR', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'AUD.ZAR', 'TP2'].iloc[-1] <= 0.25) and (switchflag4 in ('yes', 'YES')):
            
            Thread(target=NY, args=(AUDZAR_params, audzarids,26,27,ny_df)).start()
        ##################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'DKK.NOK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'DKK.NOK', 'TP2'].iloc[-1] <= 0.25) and (switchflag5 in ('yes', 'YES')):
            
            Thread(target=NY, args=(DKKNOK_params, dkknokids,28,29,ny_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.HUF', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.HUF', 'TP2'].iloc[-1] <= 0.25) and (switchflag6 in ('yes', 'YES')):

            Thread(target=NY, args=(EURHUF_params, eurhufids,30,31,ny_df)).start()
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.CZK', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'EUR.CZK', 'TP2'].iloc[-1] <= 0.25) and (switchflag7 in ('yes', 'YES')):
            Thread(target=NY, args=(EURCZK_params, eurczkids,32,33,ny_df)).start()
        ##################################################
        ...
        if (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.MXN', 'TP1'].iloc[-1] <= 0.25) and \
            (0 <= df_switch.loc[df_switch.CURRENCY == 'GBP.MXN', 'TP2'].iloc[-1] <= 0.25) and (switchflag8 in ('yes', 'YES')):
            Thread(target=NY, args=(GBPMXN_params, gbpmxnids,34,35,ny_df)).start()
        ###############################################################
...
#     df = pd.read_excel("E:\database\FX_DAILY/DATA_SWITCH.xlsx", "COND")
...
#         df_port.to_csv("E:\database\FX_DAILY/FOREX_PORTFOLIO.csv")
#         df_net.to_csv("E:\database\FX_DAILY/FOREX_NET.csv")
#         time.sleep(10)
...
# df_port=pd.read_csv("E:\database\FX_DAILY/FOREX_PORTFOLIO.csv")
...
#
# print(audnzd_usd,eurnzd_usd,noksek_usd)