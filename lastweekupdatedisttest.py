...
def run_london():
    ...
    if shortsum > 1 and longsum > 1:
        ...
        currtime = datetime.now().strftime('%H:%M:%S')
        if currtime < "20:49:25":
            sleeptime = (
                    datetime.strptime("20:49:26", '%H:%M:%S') - datetime.strptime(currtime, '%H:%M:%S')).total_seconds()
            print("LONDON ENTRIES WILL START AT 22:49, SECS LEFT: ", sleeptime)
            time.sleep(sleeptime)
