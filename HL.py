...
def LONDON(dat,params, ids, tickerId, histId, london_df):
    ...
    london_df = london_df[["CYCLE_DATE", "CURRENCY", "OPEN_POSITION", "ASK", "BID", "CLOSE",
                           "ENTRY_SIGNAL","ENTRY_TYPE", "ENTRY_QTY", "ENTRY_EXE", "ENTRY_TIME",
    ...
    if shortsum > 1 and longsum > 1:
        ...
        if currtime < "14:59:20":
            sleeptime = (datetime.strptime("14:59:21", '%H:%M:%S') - datetime.strptime(currtime,
                                                                                       '%H:%M:%S')).total_seconds()
            print("NY cycle is sleeping till scheduled time 14:59", sleeptime)
            time.sleep(sleeptime)
