...
def LONDON(dat, params, ids, tickerId, histId,london_exit_df):
    if params[0] == "AUD.NZD":
        ...
        AUDNZD_report = ["null"] * 29
        temp = AUDNZD_report

    if params[0] == "CHF.NOK":
        global CHFNOK_report
        CHFNOK_report = ["null"] * 29
        temp = CHFNOK_report

    if params[0] == "CHF.SEK":
        global CHFSEK_report
        CHFSEK_report = ["null"] * 29
        temp = CHFSEK_report

    if params[0] == "NZD.CHF":
        global NZDCHF_report
        NZDCHF_report = ["null"] * 29
        temp = NZDCHF_report

    if params[0] == "GBP.SEK":
        global GBPSEK_report
        GBPSEK_report = ["null"] * 29
        temp = GBPSEK_report

    if params[0] == "EUR.NZD":
        global EURNZD_report
        EURNZD_report = ["null"] * 29
        temp = EURNZD_report

    if params[0] == "EUR.NOK":
        global EURNOK_report
        EURNOK_report = ["null"] * 29
        temp = EURNOK_report

    if params[0] == "EUR.SEK":
        global EURSEK_report
        EURSEK_report = ["null"] * 29
        temp = EURSEK_report

    if params[0] == "GBP.NOK":
        global GBPNOK_report
        GBPNOK_report = ["null"] * 29
        temp = GBPNOK_report

    if params[0] == "NOK.SEK":
        global NOKSEK_report
        NOKSEK_report = ["null"] * 29
        temp = NOKSEK_report
...
def ldn_report():
    ...
    for i in range(500):
        time.sleep(600)
        df = pd.DataFrame(
            columns=["CURRENCY", "CYCLE_DATE", "OPEN_POSITION", "ASK", "BID", "CLOSE", "ENTRY_SIGNAL",
                      ...
                      "BUY_FILL_TYPE","SELL_MID", "SELL_FILL_TYPE", "HIGH", "LOW"])
        ...
        for i in curr_reports:
            df = df.append(pd.Series(i,
                                     ["CURRENCY", "CYCLE_DATE", "OPEN_POSITION", "ASK", "BID", "CLOSE", "ENTRY_SIGNAL",
                                      ...
                                      "BUY_EXIT_EXE", "BUY_EXIT_TIME","SELL_TP1", "SELL_TP2", "SELL_EXIT_QTY",
                                      "SELL_EXIT_EXE", "SELL_EXIT_TIME","ENTRY_BASED_TP_BUY", "ENTRY_BASED_TP_SELL", "BUY_MID",
                                      "BUY_FILL_TYPE","SELL_MID", "SELL_FILL_TYPE", "HIGH", "LOW"]), ignore_index=True)
        ...
        # df[["CURRENCY", "OPEN_POSITION", "ENTRY_QTY", "ENTRY_EXE", "BUY_TP1", "BUY_TP2", "SELL_TP1", "SELL_TP2",
        #     "BUY_EXIT_QTY", "BUY_EXIT_EXE", "SELL_EXIT_QTY", "SELL_EXIT_EXE"]].to_html(path, justify="center",
        #                                                                                index=False)
        #
        #
        #
        ...
        # def colorize(num):
        #     color = 'red' if (np.isnan(num) or num < 0) else 'rgb(34,139,34)'
