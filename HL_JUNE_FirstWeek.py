...
def london1():
    ...
    if params[0] == "AUDNZD":
        ...
        AUDNZD_report = ["null"] * 21
        temp = AUDNZD_report
        
    if params[0] == "CHFNOK":
        global CHFNOK_report
        CHFNOK_report = ["null"] * 21
        temp = CHFNOK_report

    
    if params[0] == "CHFSEK":
        global CHFSEK_report
        CHFSEK_report = ["null"] * 21
        temp = CHFSEK_report
        
    if params[0] == "NZDCHF":
        global NZDCHF_report
        NZDCHF_report = ["null"] * 21
        temp = NZDCHF_report
        
    if params[0] == "GBPSEK":
        global GBPSEK_report
        GBPSEK_report = ["null"] * 21
        temp = GBPSEK_report
        
    if params[0] == "EURNZD":
        global EURNZD_report
        EURNZD_report = ["null"] * 21
        temp = EURNZD_report
        
    if params[0] == "EURNOK":
        global EURNOK_report
        EURNOK_report = ["null"] * 21
        temp = EURNOK_report
        
        
    if params[0] == "EURSEK":
        global EURSEK_report
        EURSEK_report = ["null"] * 21
        temp = EURSEK_report
        
    
    if params[0] == "GBPNOK":
        global GBPNOK_report
        GBPNOK_report = ["null"] * 21
        temp = GBPNOK_report
    
    if params[0] == "NOKSEK":
        global NOKSEK_report
        NOKSEK_report = ["null"] * 21
        temp = NOKSEK_report
...
def report():
    ...
    df = pd.DataFrame(columns=["CURRENCY","CYCLE_DATE","OPEN_POSITION","ASK","BID","CLOSE","ENTRY_SIGNAL","ENTRY_QTY","ENTRY_TYPE","ENTRY_EXE","ENTRY_TIME",
                                     "BUY_TP1","BUY_TP2","BUY_EXIT_QTY","BUY_EXIT_EXE","BUY_EXIT_TIME"
                                     ,"SELL_TP1","SELL_TP2","SELL_EXIT_QTY","SELL_EXIT_EXE","SELL_EXIT_TIME"
                                     "ENTRY_BASED_TP_BUY","ENTRY_BASED_TP_SELL"])

    ##### make a list of all curr report list variable name   
    curr_reports = [AUDNZD_report, CHFNOK_report,CHFSEK_report,NZDCHF_report,GBPSEK_report,EURNZD_report,EURNOK_report,EURSEK_report,GBPNOK_report,NOKSEK_report]

    for i in curr_reports:
        df = df.append(pd.Series(i, ["CURRENCY","CYCLE_DATE","OPEN_POSITION","ASK","BID","CLOSE","ENTRY_SIGNAL","ENTRY_QTY","ENTRY_TYPE","ENTRY_EXE","ENTRY_TIME",
                                     "BUY_TP1","BUY_TP2","BUY_EXIT_QTY","BUY_EXIT_EXE","BUY_EXIT_TIME"
                                     ,"SELL_TP1","SELL_TP2","SELL_EXIT_QTY","SELL_EXIT_EXE","SELL_EXIT_TIME"
                                     "ENTRY_BASED_TP_BUY","ENTRY_BASED_TP_SELL"]), ignore_index=True)

    print(" ***********************************************************************")
    print(" ***********************************EVERY 15 mins DF ************************************")
    print(" ***********************************************************************")
    print(df)
