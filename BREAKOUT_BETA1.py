...
def LONDON(contract,dat, params, ids, tickerId, histId, london_df, signal, exit_signal):
    lag_val = 1
    MA_val = 20
            ...
            if confirm[confirm.orderId == id].empty == False:
                ...
                if remain > 0:
                    ...
                    if (dat.empty) or (dat[(dat["Local Symbol"] == params[0])].empty) or (
                        ...
                        print(params[0], " ENTRY PLACED @", filled, exit_exeprice)
                        break
...
def run_london():
    ...
    Thread(target=LONDON, args=(contract_eurczk,dat, EURCZK_params, ids, 100, 200, BO_df, long_params[0][1], long_params[0][2])).start()
    # ################################################
